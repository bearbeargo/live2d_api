...
$cdn_img = 'https://img.b.cr/bearbeargo/live2d-api/model/';
$cdn_others = ''; // There is something wrong with others cdn
$img_path = '../model/';
$others_path = '../model/';
if ($cdn_img != '') {
    $img_path = $cdn_img;
...
if ($cdn_others != '') {
    $others_path = $cdn_others;
}
foreach ($json['textures'] as $k => $texture)
    $json['textures'][$k] = $img_path . $modelName . '/' . $texture;

$json['model'] = '../model/' . $modelName . '/' . $json['model'];
if (isset($json['pose'])) $json['pose'] = $others_path . $modelName . '/' . $json['pose'];
if (isset($json['physics'])) $json['physics'] = $others_path . $modelName . '/' . $json['physics'];

if (isset($json['motions']))
    foreach ($json['motions'] as $k => $v) foreach ($v as $k2 => $v2) foreach ($v2 as $k3 => $motion)
        if ($k3 == 'file' || $k3 == 'sound') $json['motions'][$k][$k2][$k3] = $others_path . $modelName . '/' . $motion;

if (isset($json['expressions']))
    foreach ($json['expressions'] as $k => $v) foreach ($v as $k2 => $expression)
        if ($k2 == 'file') $json['expressions'][$k][$k2] = $others_path . $modelName . '/' . $expression;
